      ...
      {particles.map((p) => (
        <motion.div
          ...
          animate={{
            ...
            x: [`${p.x}vw`, `${p.x + p.swayOffset}vw`], // precalculated sway
          }}
          transition={{
            duration: p.duration,
            repeat: Infinity,
        ...
        >
          {p.type === 'petal' && (
            <div className={`w-3 h-4 rounded-full ${p.isDarkPetal ? 'bg-burgundy/40' : 'bg-red-900/30'} blur-[1px] transform rotate-45`} />
          )}
          {p.type === 'heart' && (

...
export default function Envelope({ onNext, onFirstTap }: { onNext: () => void; onFirstTap: () => void }) {
  ...
  const [backgroundDrifts] = useState(() => 
    Array.from({ length: 6 }).map((_, idx) => ({
      id: idx,
      startY: Math.random() * 450 + 100, // Precalculated stable pixels
  ...
  const handleOpen = () => {
    ...
    const generated = Array.from({ length: 40 }).map((_, idx) => {
      ...
      const targetY = Math.sin(angle) * distance - 200; // Radial shoot with rising offset
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        style={{ backgroundImage: 'url("/images/rice-paper-2.png")' }}
      />
      ...
      {backgroundDrifts.map((drill) => (
        <motion.div
          ...
          initial={{
            x: -50,
            y: drill.startY,
            rotate: drill.startRotate,
            scale: drill.scale,
          }}
          animate={{
            x: '105vw',
            y: `+=150`,
            rotate: drill.rotateEnd,
          }}
          transition={{
        ...
        {bloomingFlowers.map((flower) => {
          return (
            <motion.div
              ...
              animate={{
                x: [0, flower.targetX],
                y: [0, flower.targetY, flower.targetY + 160], // Arc trajectory
                opacity: [0, 1, 1, 0],
                scale: [0, 1.25, 0.95, 0],
